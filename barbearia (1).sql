...
CREATE TABLE IF NOT EXISTS tb_agendas (
    ...
    horario time, -- Hora da agenda
    dataag date, -- Data da agenda
    statusa varchar(1) DEFAULT 'A' check(statusa in ('A', 'M')), -- Correção na cláusula check
    codprof_fk int, -- Chave estrangeira que se relaciona com a tabela de profissionais
    procedimento int, -- Chave estrangeira que se relaciona com a tabela de serviços
    FOREIGN KEY (codprof_fk) REFERENCES tb_profissionais(codprof), -- Chave estrangeira para profissionais
    FOREIGN KEY (procedimento) REFERENCES tb_servicos(codservico) -- Chave estrangeira para serviços
);

-- Tabela de associação entre profissionais e serviços
DROP TABLE IF EXISTS tb_profissionais_servicos;
CREATE TABLE IF NOT EXISTS tb_profissionais_servicos (
